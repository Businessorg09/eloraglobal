...
export async function updateUserSecurityAction(userId: string, password?: string, pin?: string) {
  ...
  if (pin) {
    ...
    const { error } = await adminDb
      .from('users')
      .update({ transaction_pin_hash: hashedPin })
      .eq('id', userId)
...
export async function updatePackageAction(userId: string, packageId: string) {
  ...
  // 2. Fetch user's binary node
  const { data: node, error: nErr } = await adminDb.from('binary_nodes').select('id, is_active').eq('user_id', userId).single()
  if (nErr || !node) throw new Error('User does not have a registered network node.')

  // 3. Fetch or create user_ranks
  const { data: rank } = await adminDb.from('user_ranks').select('current_rank').eq('user_id', userId).maybeSingle()
  ...
  // 4. Update binary node and volumes
  if (!node.is_active) {
    await adminDb.from('binary_nodes').update({ is_active: true }).eq('id', node.id)
  }
  ...
  const meta = { status: 'APPROVED', approved_by: 'admin', approved_at: new Date().toISOString() };
  await adminDb.from('package_purchases').insert({
    user_id: userId,
    package_id: packageId,
    amount_paid_paise: pkg.price_inr,
    bv_generated: pkg.business_volume,
    purchase_type: node.is_active ? 'UPGRADE' : 'INITIAL',
    status: 'APPROVED',
    payment_gateway_id: JSON.stringify(meta)
  })

  // 6. Propagate BV
  if (pkg.business_volume > 0) {
    const { propagateBv } = await import('@/lib/engines/binary-tree');
    await propagateBv(adminDb, node.id, pkg.business_volume);
  }

  // 7. Increment sponsor's active direct count if it's their FIRST package
  if (!node.is_active) {
    const { data: profile } = await adminDb.from('users').select('referred_by').eq('id', userId).single()
    if (profile?.referred_by) {
      const { data: sponsorRank } = await adminDb.from('user_ranks').select('active_direct_count').eq('user_id', profile.referred_by).maybeSingle()
      if (sponsorRank) {
        await adminDb.from('user_ranks').update({ active_direct_count: (sponsorRank.active_direct_count || 0) + 1 }).eq('user_id', profile.referred_by)
      }
    }
  }
...
export async function adjustUserBalanceAction(userId: string, amountPaise: number, isAddition: boolean) {
  ...
  if (walletAcc && adminAcc) {
    ...
    if (isAddition) {
      await adminDb.from('wallet_topups').insert({
        user_id: userId,
        amount_paise: amountPaise,
        transaction_reference: `ADMIN_ADJUSTMENT_${Date.now()}`,
        status: 'APPROVED',
        approved_at: new Date().toISOString()
      })
    } else {
      const { data: wallet } = await adminDb.from('wallets').select('id, total_withdrawn_paise').eq('user_id', userId).maybeSingle()
      if (wallet) {
        await adminDb.from('wallets').update({
          total_withdrawn_paise: Number(wallet.total_withdrawn_paise || 0) + amountPaise
        }).eq('id', wallet.id)
      } else {
        await adminDb.from('wallets').insert({
          user_id: userId,
          total_withdrawn_paise: amountPaise,
